...
// Instanciation unique utilisee par Whisper large-v3-turbo.

template [[host_name("steel_attention_float32_bq32_bk32_bd64_wm4_wn1_maskfloat32")]]
[[kernel]] decltype(attention<float, 32, 32, 64, 4, 1, float>)
attention<float, 32, 32, 64, 4, 1, float>;

// Variante causale d256 pour le prefill 35B. Elle derive le kernel Steel BD64 :
// QK accumule les 4 sous-tuiles D avant softmax, puis applique la meme tuile de
// scores aux 4 sous-tuiles V. Le footprint threadgroup reste sous 32 KiB.
[[kernel, max_total_threads_per_threadgroup(4 * 1 * 32)]] void
steel_attention_float32_bq32_bk64_bd64x4_wm4_wn1_causal_d256(
  ...
  constexpr short BD = 64;
  constexpr short D_BLOCKS = 4;
  constexpr short WM = 4;
  ...
  MMATile<AccumType, TQ, TK, MMAFrag_acc_t> Stile;
  MMATile<AccumType, 1, 1, MMAFrag_acc_t> Vtile;
  MMATile<AccumType, TQ, TD, MMAFrag_acc_t> Otile0;
  MMATile<AccumType, TQ, TD, MMAFrag_acc_t> Otile1;
  MMATile<AccumType, TQ, TD, MMAFrag_acc_t> Otile2;
  MMATile<AccumType, TQ, TD, MMAFrag_acc_t> Otile3;
  Otile0.clear();
  Otile1.clear();
  Otile2.clear();
  Otile3.clear();
  ...
  for (int kb = 0; kb < kb_lim; kb++) {
    ...
    STEEL_PRAGMA_UNROLL
    for (short db = 0; db < D_BLOCKS; db++) {
      const device T* Qd = Q + db * BD;
      const device T* Kd = K + kb * BK * params->K_strides[2] + db * BD;
      QBlockLoader loader_q(
          Qd, params->Q_strides[2], Qs, simd_group_id, simd_lane_id);
      KBlockLoader loader_k(
          Kd, params->K_strides[2], Ks, simd_group_id, simd_lane_id);
    ...
    Otile0.template row_bin_op<MulOp>(factor);
    Otile1.template row_bin_op<MulOp>(factor);
    Otile2.template row_bin_op<MulOp>(factor);
    Otile3.template row_bin_op<MulOp>(factor);

    STEEL_PRAGMA_UNROLL
    for (short db = 0; db < D_BLOCKS; db++) {
      const device T* Vd = V + kb * BK * params->V_strides[2] + db * BD;
      VBlockLoader loader_v(
          Vd, params->V_strides[2], Vs, simd_group_id, simd_lane_id);
      threadgroup_barrier(mem_flags::mem_threadgroup);
      if (!align_K && kb == params->NK_aligned) {
        loader_v.load_safe(short2(BD, params->kL_rem));
      } else {
      ...
      for (short iq = 0; iq < TQ; iq++) {
        ...
        for (short id = 0; id < TD; id++) {
          ...
          for (short ik = 0; ik < TK; ik++) {
            ...
            if (db == 0) {
              MMAFrag_acc_t::mma(
                  Otile0.frag_at(iq, id),
                  Stile.frag_at(iq, ik),
                  Vtile.frag_at(0, 0),
                  Otile0.frag_at(iq, id));
            } else if (db == 1) {
              MMAFrag_acc_t::mma(
                  Otile1.frag_at(iq, id),
                  Stile.frag_at(iq, ik),
                  Vtile.frag_at(0, 0),
                  Otile1.frag_at(iq, id));
            } else if (db == 2) {
              MMAFrag_acc_t::mma(
                  Otile2.frag_at(iq, id),
                  Stile.frag_at(iq, ik),
                  Vtile.frag_at(0, 0),
                  Otile2.frag_at(iq, id));
            } else {
              MMAFrag_acc_t::mma(
                  Otile3.frag_at(iq, id),
                  Stile.frag_at(iq, ik),
                  Vtile.frag_at(0, 0),
                  Otile3.frag_at(iq, id));
            }
  ...
  Otile0.template row_bin_op<DivOp>(sum_score);
  Otile1.template row_bin_op<DivOp>(sum_score);
  Otile2.template row_bin_op<DivOp>(sum_score);
  Otile3.template row_bin_op<DivOp>(sum_score);
  threadgroup_barrier(mem_flags::mem_none);

  device T* O0 = O + (tm + sm) * params->O_strides[2] + sn;
  device T* O1 = O + BD + (tm + sm) * params->O_strides[2] + sn;
  device T* O2 = O + 2 * BD + (tm + sm) * params->O_strides[2] + sn;
  device T* O3 = O + 3 * BD + (tm + sm) * params->O_strides[2] + sn;

  if (!align_Q && int(tid.x) == params->NQ_aligned) {
    auto dst_tile_dims = short2(BD - sn, params->qL_rem - (tm + sm));
    if (dst_tile_dims.x <= 0 || dst_tile_dims.y <= 0)
      return;
    Otile0.template store_safe<T, 1, 1>(
        O0, params->O_strides[2], dst_tile_dims);
    Otile1.template store_safe<T, 1, 1>(
        O1, params->O_strides[2], dst_tile_dims);
    Otile2.template store_safe<T, 1, 1>(
        O2, params->O_strides[2], dst_tile_dims);
    Otile3.template store_safe<T, 1, 1>(
        O3, params->O_strides[2], dst_tile_dims);
  } else {
    Otile0.template store<T, 1, 1>(O0, params->O_strides[2]);
    Otile1.template store<T, 1, 1>(O1, params->O_strides[2]);
    Otile2.template store<T, 1, 1>(O2, params->O_strides[2]);
    Otile3.template store<T, 1, 1>(O3, params->O_strides[2]);
  }
}
