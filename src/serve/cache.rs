//! Cache de préfixe par blocs et garde mémoire de `serve`.
...
use saragossa::runtime_flags::{
    serve_memory_headroom_bytes, serve_memory_static_cap_bytes, serve_oom_guard_enabled,
    serve_prefix_block_tokens, serve_prefix_cache_blocks,
};
...
pub(super) struct MemoryProjection {
...
/// Garde mémoire prédictive du serveur.
#[derive(Clone, Debug)]
pub(super) struct ServeMemoryGuard {
    enabled: bool,
    static_cap: Option<u64>,
    dynamic_cap: Option<u64>,
    metal_cap: Option<u64>,
}

impl ServeMemoryGuard {
    /// Construit une garde depuis l'environnement et le device Metal.
    #[must_use]
    pub(super) fn new() -> Self {
        let enabled = serve_oom_guard_enabled();
        Self {
            enabled,
            static_cap: serve_memory_static_cap_bytes().or_else(host_memory_cap_bytes),
            dynamic_cap: host_memory_dynamic_cap_bytes(),
            metal_cap: recommended_metal_working_set_bytes(),
        }
    }

    /// Renvoie `true` si la projection dépasse le plafond.
    pub(super) fn projection_over_limit(&self, additional: u64) -> Option<MemoryProjection> {
        if !self.enabled {
            return None;
        }
        let current = process_footprint_bytes()?;
        let limit = self.limit_bytes()?;
        let projected = current.saturating_add(additional);
        (projected > limit).then_some(MemoryProjection {
            current,
            projected,
            limit,
        })
...
/// Estime la taille d'un modèle à partir des shards safetensors.
pub(super) fn estimate_model_bytes(path: &Path) -> u64 {
...
fn host_memory_dynamic_cap_bytes() -> Option<u64> {
    let memory = host_memory_bytes()?;
    Some(memory.saturating_sub(serve_memory_headroom_bytes()))
}
...
#[allow(
    unsafe_code,
    reason = "FFI Mach task_info : lit phys_footprint du process pour la garde OOM du serveur"
)]
